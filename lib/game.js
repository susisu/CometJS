            ...
            if (this._state = GameState.WAITING) {
                ...
                for (var i = 0; i < this._threads.length; i++) {
                    ...
                    this._threads[i].addEventListener(
                        comet.thread.ThreadScoreEvent.UPDATE,
...
function onThreadScoreUpdate(event) {
    this._score = this._score.update(event.info);
    this.dispatchEvent(new GameEvent(GameEvent.SCORE_UPDATE));
}
